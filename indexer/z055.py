...
def search_blocks(progress_bar):
    ...
    folder_name = f"search{search_start_time}"
                ...
                if not block_info.startswith("Error"):
                    ...
                    if hits_in_block:
                        for hit in hits_in_block:
                            ...
                            hit_filename = os.path.join(folder_name, f"{hit[0]}_hit{hit_count:08d}.txt")
                            with open(hit_filename, 'w') as hit_file:
                                hit_file.write(f"Block Height: {block_height}\n")
                                hit_file.write(f"Block Hash: {hit[1]}\n")
                            hit_count += 1
                    else:
                        result_text.insert(tk.END, "No search terms found.\n")
